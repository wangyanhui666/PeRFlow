...
def main(args):
    ...
    transform = transforms.Compose([
            transforms.Resize(512),  # 或者其他您需要的尺寸
            transforms.ToTensor()
    ])
    train_dataset = torchvision.datasets.CIFAR10(
        root='./data',  # 数据存储路径
        train=True,     # 指定获取训练集
        download=True,  # 如果数据不在root指定的路径下，自动下载
        transform=transform  # 应用上述的转换
    )
    ...
    while global_step < args.max_train_steps:
        for batch in train_dataloader:
            # process batch
            batch = {"pixel_values": batch[0],"input_ids": batch[1]}
            ## 6.0 sanity check
            ...
            with accelerator.accumulate(unet):
                ...
                with torch.no_grad():
                    ...
                    # expend the prompt_ids to the same size as latents
                    # 假设 prompt_ids 是一个一维张量，其大小为 [batch_size]
                    bs = prompt_ids.size(0)  # 获取 batch_size

                    # 首先使用 unsqueeze 增加一个维度，使其变为 [bs, 1]
                    expanded_ids = prompt_ids.unsqueeze(1)

                    # 然后使用 repeat 方法重复扩展该维度
                    prompt_ids = expanded_ids.repeat(1, 77)  # 重复 77 次
